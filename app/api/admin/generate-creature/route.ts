...
export async function GET(request: Request) {
  ...
  return Response.json(
    { enabled: query.get("cluster") === "devnet" && generationEnabled() },
...
export async function POST(request: Request) {
  ...
  if (
    !parsed.success ||
    !verifyGenerateChallenge(parsed.data, new URL(request.url).origin)
  ) {
    return Response.json(
      {
        error: {
          code: "FORBIDDEN",
          message:
            "Creature generation requires a signed Devnet wallet request.",
        },
      },
      { status: 403 },
    );
  ...
  try {
    ...
    const { data, error } = await supabase
      .from("species")
      .select("id")
      .eq("id", Number(catalogueId))
      .eq("is_active", true)
      .eq("capture_enabled", true)
      .maybeSingle();
    if (error) throw error;
    const [speciesAddress] = await findSpeciesConfigPda({ catalogueId });
    const config = await fetchMaybeSpeciesConfig(client.rpc, speciesAddress, {
    ...
    if (
      ...
      !config.exists ||
      !config.data.active ||
      config.data.catalogueId !== catalogueId
    ) {
      return Response.json(
        {
          error: { message: "This Creature is not available for generation." },
        },
        { status: 422 },
      );
    }
    const [creatureAddress] = await findCreaturePda({
      owner: address(parsed.data.wallet),
      catalogueId,
    });
    if (
      (
        await fetchMaybeCreature(client.rpc, creatureAddress, {
          commitment: "confirmed",
        })
      ).exists
    ) {
      return Response.json(
        {
          error: {
            message: "You already own this Creature. Refresh your collection.",
          },
        },
        { status: 409 },
      );
